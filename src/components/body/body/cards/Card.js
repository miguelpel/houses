...
class Card extends Component {
	...
	componentDidMount = () => {
		db.getCard(this.state.houseId, data =>
			this.setState(
				{
					data: data.val()
				},
			...
			<div className="likeBar">
				<p>
					<LikeButton
						...
						ownerId={this.state.data.userId}
					...
					<HateButton
						...
						userId={this.props.user ? this.props.user.uid : null}
					/>{' '}
					<span className="ad" id={`ad${this.state.houseId}`} />
				</p>
				{opinion !== null ? (
					<RemoveOpinion opinion={opinion} houseId={this.state.houseId} userId={this.props.user.uid} />
				) : (
					''
				)}
			</div>
